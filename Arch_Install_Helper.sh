...
confirm_action "Creating LUKS container (this may take a while)"
cryptsetup luksFormat ${DISK}p3 <<< "$LUKS_PASS"
cryptsetup open ${DISK}p3 cryptroot <<< "$LUKS_PASS"

# Create Btrfs filesystem
confirm_action "Creating Btrfs filesystem"
...
mkdir -p /mnt/{home,var,tmp,boot,boot/efi,.snapshots}
...
if [[ -n $MICROCODE ]]; then
    log "Detected CPU: $CPU_VENDOR, installing $MICROCODE"
    pacstrap /mnt base base-devel $KERNEL linux-firmware $MICROCODE btrfs-progs snapper \
        vim nano sudo cryptsetup sbctl efibootmgr iwd
else
    log "CPU vendor not detected, skipping microcode"
    pacstrap /mnt base base-devel $KERNEL linux-firmware btrfs-progs snapper \
        vim nano sudo cryptsetup sbctl efibootmgr iwd
fi
...
# Add Btrfs mount options and LUKS timeout to fstab
log "Adding Btrfs mount options and LUKS timeout to fstab..."
sed -i '/btrfs/ s/relatime/noatime,compress=zstd/' /mnt/etc/fstab
sed -i '/cryptroot/ s/defaults/defaults,x-systemd.device-timeout=0/' /mnt/etc/fstab
...
# Configure mkinitcpio for LUKS - add hooks if not present
CURRENT_HOOKS=\$(grep '^HOOKS=' /etc/mkinitcpio.conf | cut -d'=' -f2)
if [[ ! \$CURRENT_HOOKS =~ encrypt ]]; then
    ...
    echo "Updated mkinitcpio hooks for LUKS encryption"
else
    echo "LUKS encrypt hook already present in mkinitcpio.conf"
fi
mkinitcpio -P
...
options cryptdevice=UUID=\$CRYPT_UUID:cryptroot root=/dev/mapper/cryptroot rootflags=subvol=@ rw
EOL
...
log "Create network configuration files in /etc/systemd/network/"
...
log "- LUKS encryption on ${DISK}p3"
log "- Separate boot partition on ${DISK}p2"
...
log "2. Configure network in /etc/systemd/network/"
log "3. Run the post-install script in your home directory"
